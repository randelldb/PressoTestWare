...
**TL;DR;**: keep the ``search_path`` variable set to its default of ``templates``,
name schemas **other** than ``templates`` explicitly within ``Table`` definitions.
...
illustrates this behavior::
    ...
    CREATE TABLE
    test=> SET search_path TO templates, test_schema;
...
of ``templates``::
    ...
    test=> SET search_path TO templates;
    SET
...
reflection process as follows::
    ...
    >>> with engine.connect() as conn:
    ...     conn.execute("SET search_path TO test_schema, templates")
    ...     meta = MetaData()
...
.. sidebar:: Best Practices for PostgreSQL Schema reflection
    ...
    to its default of ``templates`` only, never refer to the name ``templates`` as
...
installation, this is the name ``templates``.  So a table that refers to another
which is in the ``templates`` (i.e. default) schema will always have the
...
class PGInspector(reflection.Inspector):
    ...
    def get_enums(self, schema=None):
        ...
        :param schema: schema name.  If None, the default schema
         (typically 'templates') is used.  May also be set to '*' to
