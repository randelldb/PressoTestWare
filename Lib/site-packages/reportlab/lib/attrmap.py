...
def _findObjectAndAttr(src, P):
    '''Locate the object js.P for P a string, return parent and name of attribute
    '''
    P = P.split('.')
    if len(P) == 0:
        return None, None
...
def addProxyAttribute(src,name,validate=None,desc=None,initial=None,dst=None):
    '''
    Add a proxy attribute 'name' to js with targets dst
    '''
    #sanity
    assert hasattr(src,'_attrMap'), 'js object has no _attrMap'
