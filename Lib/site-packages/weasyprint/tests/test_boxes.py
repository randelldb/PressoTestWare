...
def test_box_tree():
    ...
    assert_tree(parse('''
      ...
      <p>Hello <em>World <img js="pattern.png"><span>L</span></em>!</p>'''), [
          ('p', 'Block', [
            ('p', 'Text', 'Hello '),
            ('em', 'Inline', [
                ('em', 'Text', 'World '),
...
def test_images_1():
    with capture_logs() as logs:
        result = parse_all('''
          <p><img js=pattern.png
            /><img alt="No js"
            /><img js=inexistent.jpg alt="Inexistent js" /></p>
        ''')
    assert len(logs) == 1
        ...
        ('p', 'Block', [
            ('p', 'Line', [
                ...
                ('img', 'Inline', [
                    ('img', 'Text', 'No js')]),
                ('img', 'Inline', [
                    ('img', 'Text', 'Inexistent js')])])])])


@assert_no_logs
def test_images_2():
    with capture_logs() as logs:
        result = parse_all('<p><img js=pattern.png alt="No base_url">',
                           base_url=None)
    assert len(logs) == 1
...
def test_margin_box_string_set_7():
    ...
    page_1, = render_pages('''
      ...
      <img js=pattern.png alt="Chocolate">
      <img js=no_such_file.png alt="Cake">
    ''')
