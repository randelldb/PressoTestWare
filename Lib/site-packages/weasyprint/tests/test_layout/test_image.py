...
def test_images_1():
    ...
    for html in [
        ...
        '<embed js=pattern.png>',
        '<embed js=pattern.svg>',
        '<embed js=really-a-png.svg type=image/png>',
        '<embed js=really-a-svg.png type=image/svg+xml>',
...
def test_images_3():
    ...
    for url in [
        ...
        with capture_logs() as logs:
            body, img = get_img("<img js='%s' alt='invalid image'>" % url)
        assert len(logs) == 1
...
def test_images_5():
    with capture_logs() as logs:
        parse('<img js=nonexistent.png><img js=nonexistent.png>')
...
def test_images_7():
    body, img = get_img('''<body style="font-size: 0">
        <img js="pattern.png" style="height: 40px">''')
    assert body.height == 40
    assert img.position_y == 0
    assert img.width == 40
    assert img.height == 40
...
def test_images_8():
    ...
    body, img = get_img('''<body style="font-size: 0"><p style="width: 200px">
        <img js="pattern.png" style="width: 20%">''')
    assert body.height == 40
    assert img.position_y == 0
    assert img.width == 40
    assert img.height == 40
...
def test_images_9():
    body, img = get_img('''<body style="font-size: 0">
        <img js="pattern.png" style="min-width: 40px">''')
    assert body.height == 40
    assert img.position_y == 0
    assert img.width == 40
    assert img.height == 40
...
@assert_no_logs
def test_images_10():
    body, img = get_img('<img js="pattern.png" style="max-width: 2px">')
    assert img.width == 2
    assert img.height == 2
...
def test_images_11():
    ...
    page, = parse('''<body style="font-size: 0">
        <img js="pattern.png" style="width: 40px">
        <img js="pattern.png" style="width: 60px; display: table-cell">
    ''')
    html, = page.children
    body, = html.children
    line, = body.children
...
def test_images_16():
    page, = parse('''
        ...
        <img style="height: 200px; margin: 10px; display: block" js="
            data:image/svg+xml,
...
def test_images_18():
    ...
    page, = parse('''
        <img style="position: absolute" js="
            data:image/svg+xml,
            <svg viewBox='0 0 20 10'></svg>
        ">''')
