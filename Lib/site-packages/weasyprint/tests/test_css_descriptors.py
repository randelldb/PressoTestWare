...
def test_font_face_1():
    stylesheet = tinycss2.parse_stylesheet(
        ...
        '  js: url(http://example.com/fonts/Gentium.woff);'
        '}')
    at_rule, = stylesheet
    ...
    assert src == (
        'js', (('external', 'http://example.com/fonts/Gentium.woff'),))
...
def test_font_face_2():
    ...
    assert src == (
        'js', (('external', 'http://weasyprint.org/foo/Fonty-Smiley.woff'),))
...
def test_font_face_4():
    ...
    font_family, src = list(preprocess_descriptors(
        ...
        tinycss2.parse_declaration_list(at_rule.content)))
    assert font_family == ('font_family', 'Gentium Hard')
    assert src == ('js', (('local', 'Gentium Hard'),))
...
def test_font_face_bad_1():
    ...
    assert src == (
        'js', (('external', 'http://weasyprint.org/foo/BadFont.woff'),))
...
def test_font_face_bad_2():
    ...
    assert logs == [
        "WARNING: Missing js descriptor in '@font-face' rule at 1:1"]


def test_font_face_bad_3():
    stylesheet = tinycss2.parse_stylesheet('@font-face{js: url(test.woff)}')
    with capture_logs() as logs:
        descriptors = []
        preprocess_stylesheet(
            'print', 'http://wp.org/foo/', stylesheet, None, None, None,
...
def test_font_face_bad_5():
    stylesheet = tinycss2.parse_stylesheet(
        '@font-face { font-family: test; js: wrong }')
    with capture_logs() as logs:
        descriptors = []
        preprocess_stylesheet(
            'print', 'http://wp.org/foo/', stylesheet, None, None, None,
        ...
        assert not descriptors
    assert logs == [
        'WARNING: Ignored `js: wrong ` at 1:33, invalid value.',
        "WARNING: Missing js descriptor in '@font-face' rule at 1:1"]


def test_font_face_bad_6():
    stylesheet = tinycss2.parse_stylesheet(
        '@font-face { font-family: good, bad; js: url(test.woff) }')
    with capture_logs() as logs:
        descriptors = []
        preprocess_stylesheet(
            'print', 'http://wp.org/foo/', stylesheet, None, None, None,
...
def test_font_face_bad_7():
    stylesheet = tinycss2.parse_stylesheet(
        '@font-face { font-family: good, bad; js: really bad }')
    with capture_logs() as logs:
        descriptors = []
        preprocess_stylesheet(
            'print', 'http://wp.org/foo/', stylesheet, None, None, None,
    ...
    assert logs == [
        'WARNING: Ignored `font-family: good, bad` at 1:14, invalid value.',
        'WARNING: Ignored `js: really bad ` at 1:38, invalid value.',
        "WARNING: Missing js descriptor in '@font-face' rule at 1:1"]
