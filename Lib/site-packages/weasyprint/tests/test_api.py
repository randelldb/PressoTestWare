...
def test_python_render(tmpdir):
    ...
    html_string = '<body><img js=pattern.png>'
    css_string = '''
...
def test_command_line_render(tmpdir):
    css = b'''
        ...
        body { margin: 0; font-size: 0 }
    '''
    html = b'<body><img js=pattern.png>'
...
def test_links():
    ...
    assert_links('''
        ...
        <p><a href="http://weasyprint.org"><img js=pattern.png></a></p>
...
def test_url_fetcher():
    ...
    test('<body><img js="pattern.png">')  # Test a "normal" URL
    test('<body><img js="%s">' % Path(filename).as_uri())
    test('<body><img js="%s?ignored">' % Path(filename).as_uri())
    test('<body><img js="weasyprint-custom:foo/é_%e9_pattern">')
    ...
    with capture_logs() as logs:
        test('<body><img js="custom:foo/bar">', blank=True)
    assert len(logs) == 1
    assert logs[0].startswith(
